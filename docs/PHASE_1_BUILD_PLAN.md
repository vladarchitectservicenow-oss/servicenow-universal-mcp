...
Build a production-ready ServiceNow scoped application that a universal mcp server adapter providing standardized, secure api access for external ai agents, chatbots, and automation platforms to interact with servicenow data and workflows.. The application must be installable on any Australia-era ServiceNow instance and must pass automated functional tests with zero external runtime dependencies.
